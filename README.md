...
```shell
python -Xutf8 manage.py dumpdata cpmonitor -e contenttypes --exclude admin.logentry --indent 2 --settings=config.settings.local > cpmonitor/fixtures/example_fixture.json
```
...
```shell
python -Xutf8 manage.py dumpdata -e contenttypes --exclude admin.logentry --indent 2 --settings=config.settings.local > e2e_tests/database/test_database.json
```
...
python -Xutf8 manage.py dumpdata -e contenttypes --exclude admin.logentry --indent 2 --settings=config.settings.local > e2e_tests/database/${SNAPSHOT_NAME}.json
