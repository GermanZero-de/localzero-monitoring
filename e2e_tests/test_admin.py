...
def add_task(page: Page, title, parent=None):
...
def test_should_move_and_adjust_slugs_when_dragged(page: Page, admin_login):
    uid = str(uuid.uuid4())
    ...
    add_task(page, sector1)
    add_task(page, task, sector1)
    add_task(page, sub_task, task)
    add_task(page, sub_sub_task, sub_task)
...
def test_should_not_allow_move_when_same_title_in_same_sector(page: Page, admin_login):
    ...
    add_task(page, sector1)
    add_task(page, task1, sector1)

    add_task(page, sector2)
    add_task(page, task2, sector2)
